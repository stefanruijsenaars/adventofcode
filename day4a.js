...
var getKey =  function (str) {
  ...
  lettersArr.sort(function (a, b) {
    var aNum = +JSON.stringify(a).replace(/"(.*?)"\:|\{|\}/g,'' );
    var bNum = +JSON.stringify(b).replace(/"(.*?)"\:|\{|\}/g,'' );
    for (var aLetter in a) break;
    for (var bLetter in b) break;
  ...
  return lettersArr.slice(0, 5).map(function (item) {
    for (var letter in item) break;
    return letter;
  }).join('');
};
...
for (var i = 0; i < data.length; i++) {
  var room = data[i];
  var re = /(.*)-([0-9]+)\[([a-z]+)\]/;
  var matches = room.match(re);
  var letters = matches[1];
  letters = letters.replace(/-/g, '');
  var num = +matches[2];
  var key = matches[3];
  var realKey = getKey(letters);
  if (key === realKey) {
    total += num;
  }
}
