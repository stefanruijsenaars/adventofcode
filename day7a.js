...
const hasAbba = (sequence) => {
  return sequence.split('').some((letter, i) => {
    if (sequence[i + 3] !== undefined) {
...
const count = (input) => {
  input.forEach((sequence) => {
    ...
    // Check that there is at least 1 abba in the matches, and none at all in nope matches.
    if (!bracketMatches.some(hasAbba) && matches.some(hasAbba)) {
      counter++;
    }
  })
  console.log(counter);
