...
const getAbas = (sequence, abas) => {
  sequence.split('').forEach((letter, i) => {
    if (sequence[i + 2] !== undefined) {
      if (sequence[i] === sequence[i + 2]) {
        if (sequence[i] !== sequence[i + 1]) {
          abas.push(sequence[i] + sequence[i + 1] + sequence[i + 2]);
        }
      }
    }
  });
...
const count = (input) => {
  ...
  input.forEach((sequence) => {
    // Matches within brackets.
    const bracketMatches = sequence.match(/(?<=\[).+?(?=\])/g);
    // Matches outside brackets.
    const matches = sequence.replace(/(\[.*?\])/g, '|').split('|');
    const abas = matches.reduce((acc, cur) => {
      return getAbas(cur, acc);
    }, []);
    const babs = abas.map(reverse);

    let hasMatch = false;
    babs.forEach((bab) => {
      bracketMatches.forEach((bracketMatch) => {
        if (bracketMatch.indexOf(bab) !== -1) {
          hasMatch = true;
        }
      });
    });
